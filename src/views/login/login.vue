<template>
  <div :class="['login-wrapper', ['', 'login-form-right', 'login-form-left'][direction]]">
    <el-form ref="loginForm" :model="form" :rules="rules" class="login-form ele-bg-white" size="large" @keyup.enter.native="doSubmit">
      <h4>用户登录</h4>
      ...
      <el-form-item prop="code">
        <div class="login-input-group">
          ...
          <img v-if="captcha" :src="captcha" @click="changeCode" class="login-captcha" alt=""/>
        </div>
      </el-form-item>
      ...
      <div class="el-form-item">
        <el-button @click="doSubmit" :loading="loading" type="primary" class="login-btn" size="large">{{ loading ? '登录中' : '登录' }}</el-button>
      </div>
      <div class="ele-text-center" style="margin-bottom: 10px;">
        <i class="login-oauth-icon el-icon-_qq" style="background-color:#3492ed;"></i>
        <i class="login-oauth-icon el-icon-_wechat" style="background-color:#4daf29;"></i>
        <i class="login-oauth-icon el-icon-_weibo" style="background-color: #CF1900;"></i>
      </div>
    </el-form>
  ...
  methods: {
    ...
    changeCode() {
      this.$http.get('/file/captcha').then(res => {
        if (res.data.code === 0) {
          this.captcha = res.data.data.img;
