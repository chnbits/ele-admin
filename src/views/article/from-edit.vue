<!-- 角色编辑弹窗 -->
<template>
  <el-dialog :title="isUpdate?'修改来源':'添加来源'" :visible="visible" width="400px" :destroy-on-close="true" :lock-scroll="false" @update:visible="updateVisible">
    <el-form ref="form" :model="form" :rules="rules" label-width="82px">
      <el-form-item label="来源名称:" prop="name">
        <el-input v-model="form.name" placeholder="请输入来源名称" clearable/>
      </el-form-item>
      <el-form-item label="来源链接:" prop="url">
        <el-input v-model="form.url" placeholder="请输入来源链接" clearable/>
      </el-form-item>
  ...
  methods: {
    ...
    save() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          this.loading = true;
          this.$http[this.isUpdate ? 'put' : 'post']('/article/from', this.form).then(res => {
            this.loading = false;
            if (res.data.code === 0) {
